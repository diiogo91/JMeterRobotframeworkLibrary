...
class JMeterLib:
    ...
    @keyword("run jmeter")
    def runJmeter(self, testPlanPath, logFilePath, otherParams=""):
        """
            ...
            """
        self.JMeterRunner(testPlanPath, logFilePath, otherParams)

    @keyword("run jmeter analyse jtl convert")
    def runJmeterAnalyseJtlConvert(self, testPlanPath, logFilePath, otherParams="", disableReports=None):
        """
            ...
            """
        self.JMeterRunner(testPlanPath, logFilePath, otherParams)
        lai = self.LogAnalysisInitiator(logFilePath, True, True, disableReports=disableReports)
        return lai.getReturnStructure()

    @keyword("run jmeter analyse jtl convert to db")
    def runJmeterAnalyseJtlConvertToDb(self, testPlanPath, logFilePath, otherParams=""):
        """
            ...
            """
        self.JMeterRunner(testPlanPath, logFilePath, otherParams)
        lai = self.LogAnalysisInitiator(logFilePath, True)
        return lai.getReturnStructure()

    @keyword("run jmeter analyse jtl convert to html")
    def runJmeterAnalyseJtlConvertToHtml(self, testPlanPath, logFilePath, otherParams="", disableReports=None):
        """
            ...
            """
        self.JMeterRunner(testPlanPath, logFilePath, otherParams)
        lai = self.LogAnalysisInitiator(logFilePath, createHtmlReport=True, disableReports=disableReports)
        return lai.getReturnStructure()

    @keyword("run jmeter analyse jtl")
    def runJmeterAnalyseJtl(self, testPlanPath, logFilePath, otherParams=""):
        """
            ...
            """
        self.JMeterRunner(testPlanPath, logFilePath, otherParams)
        lai = self.LogAnalysisInitiator(logFilePath)
        return lai.getReturnStructure()

    @keyword("analyse jtl convert")
    ...
    @keyword("analyse jtl convert to db")
    ...
    @keyword("analyse jtl convert to html")
    ...
    def create_dir(self, folder):
        try:
            if not os.path.exists(folder):
                os.makedirs(folder, mode=0o666, exist_ok=True)
        except OSError:
            sys.exit('Fatal: output directory "' + folder + '" does not exist and cannot be created')
        ...
        try:
            ...
            while True:
                ...
                if output == '' and process.poll() is not None:
                    logger.console("Error execution subprocess")
                    raise self.JMeterLibException("%s %s" % ("ERROR:", process.stdout.readline()))
                if output:
                    logger.console(output.strip())
                if end_signal in str(output.strip()):
                    logger.console("Finishing subProcess...")
                    break
        except Exception as e:
            raise self.JMeterLibException("%s %s" % ("ERROR:", e))

    class JMeterRunner(object):
        def __init__(self, testPlanPath, logFilePath, otherParams):
        ...
        def validateInput(self):
            ...
            if not op.isfile(self.jmx):
                raise self.JMeterLibException("Wrong test plan path.")
        ...
        def clear_log_files(self):
            self.self.remove_file(self.log + LOG_JTL)

        def runAndPrintResult(self):
            self.clear_log_files()
            command = "jmeter -n -t " + self.jmx
            if len(self.params) > 0:
                for p in self.params:
                    command = command + ' ' + p
            command = command + " -l " + self.log + LOG_JTL + " -f -e -o " + self.log + "_reports"
    ...
    class LogAnalysisInitiator(object):
        ...
        def initiateNewAnalyserObject(self):
            ...
            try:
                ...
                raise self.JMeterLibException("File %s couldn't be opened" % self.jtlPath)
            else:
                if self.recognizeFormat(newFileLines) == "csv":
                    newObject = self.CsvLogAnalyser(self.jtlPath)
                    print("Log file format: csv")
                elif self.recognizeFormat(newFileLines) == "xml":
                    newObject = self.XmlLogAnalyser(self.jtlPath)
                    print("Log file format: xml")
                else:
                    raise self.JMeterLibException("Incorrect log file format")
            return newObject

        def convertLogToHtml(self, disableReports=None):
            self.lc = self.LogConverterHtml(self, disableReports=disableReports)
            self.remove_file(self.jtlPath + ".html")
            self.lc.createNewHtmlPath()
            self.lc.createHtml(disableReports)

        def convertLogToSql(self):
            self.ls = self.LogConverterSql(self)
            self.ls.createSql()
    ...
    class LogAnalyser(object):
        ...
        def calculate(self):
            ...
            self.aggrSummary = self.AggregatedSummary()
            self.aggrSamples = []
            self.totalSamples = self.AggregatedSamples("TOTAL")
            for s in self.samples:
        ...
        def checkWhichAggregated(self, name, start):
            ...
            if aggrId < 0:
                aggrId = counter + 1
                self.aggrSamples.append(self.AggregatedSamples(name, aggrId))
                self.aggrSamples[aggrId].setStartTime(start)
            return aggrId

        def convertLogToHtml(self):
            self.lc = self.LogConverterHtml(self)
            self.lc.createNewHtmlPath()
            self.lc.createHtml()

        def convertLogToSql(self, dbname, dbReady):
            self.ls = self.LogConverterSql(self, dbname, dbReady)
            self.ls.createSql()
            ...
            try:
                with open(self.filePath, "r") as csvfile:
                    ...
                    for row in csvReader:
                        ...
                        if counter == 0:
                            try:
                                if row[0].find('timeStamp') == 0 and row[1].find('elapsed') == 0 and row[0].find(
                                        'timeStamp') == 0:
                                    header_found = True
                            except Exception as e:
                                pass
                        if header_found:
                            if counter >= 1:
                                newSample = self.Sample(ts=row[0], t=row[1], lb=row[2], rc=row[3],
                            ...
                            newSample = self.Sample2(ts=row[0], t=row[1], lb=row[2], rc=row[3],
                                                     rm=row[4], tn=row[5], dt=row[6], s=row[7],
                                                     by=row[8], lt=row[9], ng=row[10], na=row[11])
                        if type(newSample) == self.Sample or type(newSample) == self.Sample2:
                            self.samples.append(newSample)
                        counter += 1
            except IOError:
                print("ERROR, problems while reading " + str(self.filePath))
            if len(self.samples) <= 0:
                raise self.JMeterLibException("No samples were found in a log file.")
                        ...
                        if isinstance(s, xml.dom.minidom.Element):
                            if self.validateXmlSampleAttributes(s):
                                if s.hasAttribute('ng') and s.hasAttribute('na'):
                                    newSample = self.Sample2(ts=s.getAttribute('ts'), t=s.getAttribute('t'),
                                    ...
                                    newSample = self.Sample(ts=s.getAttribute('ts'), t=s.getAttribute('t'),
                                                            lb=s.getAttribute('lb'), rc=s.getAttribute('rc'),
                                                            rm=s.getAttribute('rm'), tn=s.getAttribute('tn'),
                                                            dt=s.getAttribute('dt'), s=s.getAttribute('s'),
                                                            by=s.getAttribute('by'), lt=s.getAttribute('lt'))
                                ...
                                for a in assertNodes:
                                    ...
                                    if isinstance(a, xml.dom.minidom.Element):
                                        ...
                                        newAssertion = self.Assertion(name=nameTagString, failure=failureTagString,
                                                                      failureMessage=failureMessageTagString,
                                                                      error=errorTagString)
                                        newSample.addAssertion(newAssertion)
                                if isinstance(newSample, self.Sample) or isinstance(newSample, self.Sample2):
                                    self.samples.append(newSample)
            if len(self.samples) <= 0:
                raise self.JMeterLibException("No samples were found in a log file.")
    ...
    class Sample2(Sample):
        def __init__(self, **values):
            super(self.Sample2, self).__init__(**values)
            if 'ng' in values:
    ...
    class AggregatedSamples(AggregatedSummary):
        def __init__(self, name, Id=-1):
            super(self.AggregatedSamples, self).__init__()
            ...
            if type(testrunKey) == int and testrunKey > 0 and self.dbStatus:
                for agg in self.loganalyser.aggrSamples:
                    ...
                    if sqlCommand != "" and agg.sampleName != "TOTAL":
                        ...
                        aggrId = self.getIdFromSelect(sqlSelect,
                                                      "ERROR while executing \"SELECT ... FROM Aggregated\" command")
                ...
                for s in self.loganalyser.samples:
                    ...
                    if fk > 0 and type(fk) == int:
                        ...
                        if sqlCommand != "":
                            ...
                            sampleId = self.getIdFromSelect(sqlSelect,
                                                            "Error while executing \"SELECT ... FROM Sample\" command")
    ...
    class LogConverterHtml(object):
        ...
        def createHtmlAggrRepAndGraph(self):
            ...
            for agg in self.loganalyser.aggrSamples:
                if agg.getAmountOfSamples() > 1:
                    ...
                    aggHtml += "</canvas><br>\n"
                    aggHtml += self.addAggrRepJs(canvasId, aggCounter, agg.sampleName,
                                                 [agg.getAverageTime(), agg.getMedian(),
        ...
        def createHtmlAggrSamples(self):
            ...
            for agg in self.loganalyser.aggrSamples:
                if agg.link != "samples_" and agg.sampleName != "TOTAL":
                    aggHtml += "<a id=\"" + agg.link + "\"><p id=\"navifont\">" + self.htmlParts[
                        'nbspx10'] + agg.sampleName + " </p></a><br>"
                    oneTypeTable = []
        ...
        def createHtmlRespTimeGraph(self):
            ...
            for agg in self.loganalyser.aggrSamples:
                if agg.getAmountOfSamples() > 1:
                    ...
                    respHtml += " <button onclick=\"lc" + str(
                        aggCounter) + ".drawChartData()\">DRAW CHART for " + agg.sampleName + "</button><br><br>"
        ...
        def createHtmlAllSamples(self):
            return "<br><a id=\"samples_\"><p id=\"navifont\">All samples </p></a>" + self.samplesToHtml(
                self.loganalyser.samples)
